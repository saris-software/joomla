...
class BDT_bdt_display_Controller {
	// configuration array
	private $config;
	// module info
	private $module;
	...
	function checkImages() {
		...
		if($this->config['generate_thumbnails'] == 1) {
			...
			// basic images params		
			$img_width = ($display_layout == 'display') ? '507': '492';
			$img_height =($display_layout == 'display') ? '286': '309';
			$img_bg = '#000';
			$quality = $this->config['config']->bdt_display->bdt_display_quality;
			// check the slides
	...
	function generateView() {
		...
		if(!$css_founded && $this->config['use_style_css'] == 1) {
			...
			$document->addStyleSheet(BDT_MS_CORE_URL.'skins/'. $this->config['config']->bdt_display->bdt_display_skins_style.'/style.css' );
		...
		$document->addStyleSheet(BDT_MS_STYLE_URL.$this->config['styles'].'/style/ms-'.$display_layout.'-style.css' );

		// generate necessary variables
		$width = ($display_layout == 'display') ? '507': '492';
		$height = ($display_layout == 'display') ? '286': '309';
		// load view
