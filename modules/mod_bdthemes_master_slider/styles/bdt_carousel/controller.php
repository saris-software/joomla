...
class BDT_bdt_carousel_Controller {
	// configuration array
	private $config;
	// module info
	private $module;
	...
	function checkImages() {
		...
		if($this->config['generate_thumbnails'] == 1) {
			// basic images params		
			$img_width = $this->config['config']->bdt_carousel->bdt_carousel_image_width;
			$img_height = $this->config['config']->bdt_carousel->bdt_carousel_image_height;
			$img_bg = $this->config['config']->bdt_carousel->bdt_carousel_image_bg;
			$quality = $this->config['config']->bdt_carousel->bdt_carousel_quality;
			// check the slides
	...
	function generateView() {
		...
		$document->addStyleSheet(BDT_MS_CORE_URL.'skins/'. $this->config['config']->bdt_carousel->bdt_carousel_skins_style.'/style.css' );
		}

		$document->addStyleSheet(BDT_MS_STYLE_URL.$this->config['styles'].'/ms-caro3d.css' );

		// generate necessary variables
		$width = $this->config['config']->bdt_carousel->bdt_carousel_image_width;
		$height = $this->config['config']->bdt_carousel->bdt_carousel_image_height;
		// load view
