...
class BDT_bdt_simple_slider_Controller {
	// configuration array
	private $config;
	// module info
	private $module;
	...
	function checkImages() {
		...
		if($this->config['generate_thumbnails'] == 1) {
			// basic images params		
			$img_width = $this->config['config']->bdt_simple_slider->bdt_simple_slider_image_width;
			$img_height = $this->config['config']->bdt_simple_slider->bdt_simple_slider_image_height;
			$img_bg = $this->config['config']->bdt_simple_slider->bdt_simple_slider_image_bg;
			$quality = $this->config['config']->bdt_simple_slider->bdt_simple_slider_quality;
			// check the slides
	...
	function generateView() {
		...
		if(array_search(BDT_MS_CORE_URL.'skins/default/style.css', $headData_css_keys) > 0) {
			$css_founded = true;
		}
		...
		if(!$css_founded && $this->config['use_style_css'] == 1) {
			...
			$document->addStyleSheet(BDT_MS_CORE_URL.'skins/'. $this->config['config']->bdt_simple_slider->bdt_simple_slider_skins_style.'/style.css' );
		}

		// generate necessary variables
		$width = $this->config['config']->bdt_simple_slider->bdt_simple_slider_image_width;
		$height = $this->config['config']->bdt_simple_slider->bdt_simple_slider_image_height;
		// load view
