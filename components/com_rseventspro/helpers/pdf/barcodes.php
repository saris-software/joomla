...
class TCPDFBarcode {
	...
	public function getBarcodePNG($w=2, $h=30, $color=array(0,0,0)) {		
		if (strtolower($this->type) == 'qrcode') {
			$w = 6;
			$h = 6;
			...
			if ($imagick) {
				...
				$text_width = imagefontwidth(5)*strlen($this->barcode_array['code']);
				$center = ceil($width / 2);
				$thex = $center - (ceil($text_width/2)); 
				//imagestring($png , 5 , $thex , 30 , $this->barcode_array['code'] , $color );
	...
	public function setBarcode($code, $type) {
		switch (strtoupper($type)) {
			case 'QRCODE': { // QRCode
				$arrcode = $this->barcode_qrcode($code, 'Q');
				break;
			}
	...
	protected function barcode_qrcode($code, $type = 'L') {
		require_once(dirname(__FILE__).'/qrcode.php');
		
		$qrcode = new QRcode($code, $type);
		$bararray = $qrcode->getBarcodeArray();
		$bararray['code'] = $code;
