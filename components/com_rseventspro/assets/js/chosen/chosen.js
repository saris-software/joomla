...
var Chosen = new Class({
	...
	set_up_html: function(){
		...
		this.default_text = this.form_field.get('data-placeholder') ? this.form_field.get('data-placeholder') : (this.form_field.multiple ? Joomla.JText._('COM_RSEVENTSPRO_SELECT_SOME_OPTIONS') : Joomla.JText._('COM_RSEVENTSPRO_SELECT_AN_OPTION'));
	...
	result_select: function(evt){
		if (this.result_highlight){
			...
			high.addClass("result-selected").setStyle('display', 'none');;
	...
	no_results: function(terms){
		var no_results_html = new Element('li', {'class': 'no-results'}).set('html', Joomla.JText._('COM_RSEVENTSPRO_SELECT_NO_RESULTS_MATCH')+' "<span></span>"');
